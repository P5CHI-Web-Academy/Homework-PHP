...
class Import
{
    ...
    public function import_data()
    {
        $validation = [
            'date'    => function ($arg) {
                return DateTime::createFromFormat('Y-m-d', $arg) !== false;
            },
            'type'    => function ($arg) {
                return ctype_upper($arg);
            },
            'owner'   => function ($arg) {
                return ctype_lower($arg) && ctype_alnum($arg);
            },
            'message' => true  // Any
        ];
        foreach (glob(__DIR__.'/'.$this->dir.'/*.json') as $file) {
            if (($decoded = json_decode(file_get_contents($file), true))
                !== null
            ) {
                $error
                                = false; //This flag means, whether all rows in the file are valid
                $data_to_insert = [];
                foreach ($decoded as $item) {
                    if ( ! $error = (count(
                                         array_intersect_key($validation, $item)
                                     ) !== count($validation))
                    ) {
                        $data_to_insert[] = array_combine(
                            array_map(
                                function ($v) {
                                    return ':'.$v;
                                },
